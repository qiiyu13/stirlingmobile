...
| R12 | **(Resolved, W20) lopdf `Document::save()` corrupted some valid PDFs** — carried a stale `trailer["Prev"]` offset from the source file's incremental-update/linearization chain into the freshly written (differently-sized) file, breaking any reader that follows `Prev`. Cross-cutting: hit every engine tool that loads-and-saves a PDF (~30 `doc.save()` call sites), not scoped to any one tool. Found while testing Track A tools (docs/17-track-a-native-tools-status.md), confirmed via a bare `load→save` repro with zero tool code involved. | — (resolved) | — | Fixed: `save_document()` helper in `engine/src/content_util.rs` strips `Prev`/`XRefStm` before delegating to `doc.save()`; every call site routed through it. 89 engine tests still passing. | — |
