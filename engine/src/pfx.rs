...
pub fn load_pfx(pfx_path: &str, password: SecretString) -> Result<SigningIdentity, EngineError> {
    ...
    let private_key =
        RsaPrivateKey::from_pkcs8_der(&key_der).map_err(|e| EngineError::ReadFailed {
            path: pfx_path.to_string(),
...
fn safe_bags(auth_safe: &p12::ContentInfo, password: &str) -> Option<Vec<p12::SafeBag>> {
    ...
    let contents: Vec<p12::ContentInfo> = yasna::parse_der(&top_level, |r| {
        r.collect_sequence_of(p12::ContentInfo::parse)
    })
    .ok()?;
    ...
    for content in &contents {
        ...
        let safe_bags: Vec<p12::SafeBag> =
            yasna::parse_der(&data, |r| r.collect_sequence_of(p12::SafeBag::parse)).ok()?;
...
fn decrypt_algorithm(
    alg: &p12::AlgorithmIdentifier,
    ciphertext: &[u8],
    password: &str,
) -> Option<Vec<u8>> {
    match alg {
        ...
        p12::AlgorithmIdentifier::OtherAlg(other) => {
            decrypt_pbes2(other, ciphertext, password.as_bytes())
        }
...
fn decrypt_pbes2(
    other: &p12::OtherAlgorithmIdentifier,
    ciphertext: &[u8],
    password_raw: &[u8],
) -> Option<Vec<u8>> {
    use der::Decode;
